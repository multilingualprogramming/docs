...
Tous les mots-cles de multilingual se mappent vers des **concepts semantiques** - pas des tokens bruts. Le lexer relie les mots-cles de surface specifiques a la langue aux concepts ; le parser et le codegen operent uniquement sur les concepts.

---

## Systeme de concepts

Un concept comme `COND_IF` se mappe vers `if` en anglais, `si` en francais, `wenn` en allemand, `moshi` en japonais, etc. Cela maintient le parser et l'analyseur semantique independants de la langue.

Le registre des mots-cles est stocke dans :
`multilingualprogramming/resources/usm/keywords.json`

---

## Mots-cles de controle de flux
...
Les noms builtins anglais universels fonctionnent toujours. En plus, chaque langue dispose d'alias localises :
